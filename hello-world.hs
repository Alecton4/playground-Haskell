...
-- From Phind: `head' x : _ = x` will be problematic because Haskell will interpret it as `(head' x) : _ = x`.
head' (x : _) = x
...
-- From Phind: Pattern matching in Haskell works by matching the structure of data,
